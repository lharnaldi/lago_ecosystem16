...
int main()
{
	...
	unsigned char buf_o[COPY_BYTES], buf_i[COPY_BYTES];
	...
	*((uint32_t *)(cfg + 0)) |= 3;
		...
		if(position >= num_word)//  (limit > 0 && position > limit) || (limit == 0 && position < 512))
		{
			memcpy(buf_o,ram,COPY_BYTES); //copy bytes from BRAM
			//printf("buf_o = 0x0%08X\n",buf_o);
			for(i = 0; i < COPY_BYTES; i=i+4)
			{
				//printf("%5d\n", *((int32_t *)(ram + 4*i)));
				printf("%5d\n", buf_o[i]);
			}
