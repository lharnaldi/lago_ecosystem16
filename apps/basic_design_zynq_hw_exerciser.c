...
#ifdef LINUX_APP                                   // Linux application includes
#include <stdlib.h>
#include <stdio.h>
#include <fcntl.h>
#include <sys/mman.h>
...
#else                                            // standalone application includes
...
#ifdef LINUX_APP                          // Linux axi_gpio memory mapped access and initialization
void *Linux_GPIO_initialize(int gpio_base_address, int direction, int first_call);
#else                                     // Standalone axi_gpio initialization
XGpio GPIO_initialize(int device_id, int direction);
// Standalone axi_gpio global instance pointers (not needed for Linux)
XGpio LED_device; // The driver instance for GPIO Device configured as O/P, required by Standalone library services
XGpio button_device;  // The driver instance for GPIO Device configured as I/P, required by Standalone library services
#endif
...
#ifndef LINUX_APP
   // local variable (not needed for Linux)
   int delay = 0;

   // Standalone caching functions (not needed for Linux)
   Xil_ICacheEnable();
   Xil_DCacheEnable();
...
#else
   // Standalone axi_gpio initialization (not needed for Linux)
   // The associated GPIO is initialized and data direction is set
   LED_device    = GPIO_initialize(XPAR_GPIO_LEDS_8BITS_DEVICE_ID,ALL_OUTPUTS);	    // first the outputs
   button_device = GPIO_initialize(XPAR_GPIO_BUTTONS_3BITS_DEVICE_ID,ALL_INPUTS);	// then the inputs

   // if any failures during initialization, do a controlled hang, typically a problem if the GPIO does not exist
   if ((LED_device.BaseAddress == 0) || (button_device.BaseAddress == 0))
   	   { while(1) { print("Hung..."); for (delay=0; delay<10000000; delay++); } }
   else
...
#ifndef LINUX_APP                                           // Standalone caching functions (not needed for Linux)
   Xil_DCacheDisable();
   Xil_ICacheDisable();
...
/******************************************************************************************************
* STANDALONE GPIO INITIALIZATION
* This function performs two operations:
* 1) Init1alizes the GPIO specified by the device_id
* 2) Initialize the GPIO for either input or output mode.
*
* INPUT PARAMETERS:
* device_id - XPAR_GPIO_????????_DEVICE_ID from Xparameters.h file
* direction - 32 bits indicating direction for each bit; 0 - output; 1 - input
*
* RETURNS:
* address_handle - memory pointer to the GPIO that was specified by XPAR_GPIO_????????_DEVICE_ID.
*                  This is used by the various Standalone library routines to identify which GPIO
*                  is being referenced.
*******************************************************************************************************/
XGpio GPIO_initialize(int device_id, int direction) {
	 int  status;
	 char *failure_message = "Failed to initialize GPIO with a device ID of: ";
	 char *success_message = "Succeeded in initializing GPIO with a device ID of: ";
	 char *final_message   = "???";
	 XGpio address_handle;
	 failure_message[47] = success_message[51] = '\0';   // remove previous results from last call

	 // initialize the GPIO device and let the user know what happened, Standalone library service
	 status = XGpio_Initialize(&address_handle, device_id);
	 if (status == XST_SUCCESS)  { final_message = strcat(success_message,itoa(device_id,final_message,10)); }
	 else                        { final_message = strcat(failure_message,itoa(device_id,final_message,10)); }
     print(final_message); print("\n\r");

	 XGpio_SetDataDirection(&address_handle, 1, direction);                  // Set the direction for all signals to be outputs
	 if (direction == 0) { XGpio_DiscreteWrite(&address_handle, 1, 0x0); }   // Set the GPIO outputs to low if all pins are outputs
...
#else
		// Standalone library read of gpio, using a library device driver service
		current_button_state = XGpio_DiscreteRead(&button_device, BUTTON_CHANNEL);
#endif	
...
#else
		// Standalone library write to gpio, using a library device driver service
		XGpio_DiscreteWrite(&LED_device,LED_CHANNEL,led_image);
#endif
}
