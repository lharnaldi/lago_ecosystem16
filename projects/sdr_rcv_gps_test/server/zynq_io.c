...
int intc_init(void)
{
	...
	if (intc_fd < 1) {
		printf("intc_init: Invalid device file:%s.\n", uiod);
		return -1;
	}
	...
	intc_ptr = mmap(NULL, dev_size, PROT_READ|PROT_WRITE, MAP_SHARED, intc_fd, INTC_BASEADDR);
...
int cfg_init(void)
{
	...
	cfg_fd = open(uiod, O_RDWR);
	if (cfg_fd < 1) {
		printf("cfg_init: Invalid device file:%s.\n", uiod);
		return -1;
	}
...
int sts_init(void)
{
	...
	if (sts_fd < 1) {
		printf("sts_init: Invalid device file:%s.\n", uiod);
		return -1;
	}
	...
	sts_ptr = mmap(NULL, dev_size, PROT_READ|PROT_WRITE, MAP_SHARED, sts_fd, STS_BASEADDR);
...
int mem_init(void)
{
	...
	mem_ptr = mmap(NULL, dev_size, PROT_READ|PROT_WRITE, MAP_SHARED, mem_fd, RAM_BASEADDR);
